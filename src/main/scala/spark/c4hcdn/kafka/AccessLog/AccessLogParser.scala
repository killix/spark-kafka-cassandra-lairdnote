package spark.c4hcdn.kafka.AccessLog
...
class AccessLogParser extends Serializable {
    ...
    private val agent = "\"(.*?)\""
	private val hit = "(.*?)"
	private val origintime = "(\\d+?)"
    private val regex = s"$ip $client $user $dateTime $request $status $bytes $referer $agent $hit $origintime"
